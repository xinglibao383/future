...
def process(source_dir, target_dir, use_len, compute_len, predict_len, stride_len):
    ...
    for filename in os.listdir(pose_source_dir):
        ...
        if os.path.exists(imu_filepath):
            ...
            for frame_end_idx in range(use_len + predict_len, len(pose_data), stride_len):
                pose_sub_data, imu_sub_data = [], []
                frame_start_idx = frame_end_idx - (compute_len + predict_len)
                ...
                imu_end_idx = int(imu_data.shape[1] / len(pose_data) * frame_end_idx)
                imu_start_idx = imu_end_idx - int((use_len + predict_len) / 15 * 50)
                name_parts = filename.replace('.json', '').split('_')
                name_parts[1] = mapping[name_parts[1]]
...
if __name__ == "__main__":
    process("/mnt/mydata/yh/liming/data/xrfv2", "/mnt/mydata/yh/liming/workspace/future/mydata", use_len=150, compute_len=15, predict_len=15, stride_len=15)