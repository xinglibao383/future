            ...
            for frame in pose_data:
                if "frame_id" in frame and "pose_key_points:" in frame:
                    ...
                    if frame_id != 0 and frame_id % 5 == 0:
                        ...
                        np.save(os.path.join(imu_target_dir, '_'.join(name_parts) + '.npy'), slice(imu_data, imu_end_index-imu_len+1, imu_end_index))
                        np.save(os.path.join(pose_target_dir, '_'.join(name_parts) + '.npy'), np.array(frame["pose_key_points:"][0], dtype=np.float32))
            
if __name__ == "__main__":
    process("/data/xinglibao/xrfv2", "/home/xinglibao/workspace/future/mydata", 150)